...
const Profile = () => {
    ...
    /*
    If authed,
    1. pull all data from profiles table where the id == user.id
    2. If we have no errors, update local state with pulled profile data

    Dependency is on user object
    */
    useEffect(() => {
        if (user) {
            setEmail(user.email);
    ...
    /*
    Function to update specific user in profiles table.
    Using local state, we send an object with all fields
    If there's no error, we update local state
    */
    ...
    //Makes alpha numeric id at a givenlength
    ...
    const uploadHandler = async (event) => {
        ...
        const { error } = await supabaseClient.storage
            ...
            .upload(fileName, avatarFile, {
                cacheControl: "3600", //Cache for a max of 1 hour before going stale
                upsert: false, //Upsert = insert, update on conflict
            });
        if (error) {
